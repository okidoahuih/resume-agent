module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const { apiKey, job = '', resume = '', model = 'gpt-5-mini' } = payload;
    ...
    const text = data.output_text || (Array.isArray(data.output) ? data.output.flatMap(x => Array.isArray(x.content) ? x.content : []).map(x => x.text || '').join('') : '');
